...
internal func with<A: Actor, Value: ThreadConfined>(
    ...
    return try await actor.invoke { actor in
        ...
        let realm: Realm
        do {
            realm = try Realm(RLMRealm(configuration: config.wrappedValue, confinedTo: scheduler))
        } catch {
            print("Failed to initialize Realm: \(error)")
            return NotificationToken() // Return default on failure
        }

        guard let resolvedValue = tsr.resolve(in: realm) else {
            return NotificationToken() // Return default if resolving fails
        }
        ...
        // a second variable
        return try await block(actor, resolvedValue)
    } ?? NotificationToken()
}
#else
@available(macOS 10.15, tvOS 13.0, iOS 13.0, watchOS 6.0, *)
